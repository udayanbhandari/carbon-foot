...
@pytest.mark.asyncio
async def test_async_roundtrip(repo):
    data = CarbonInput()
    created = await repo.async_add("device-fire-async", data, calculate_footprint(data))
    assert created.id
    listed = await repo.async_list_for_device("device-fire-async")
    assert len(listed) == 1
